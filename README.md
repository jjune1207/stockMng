...
- **종목 구분 필터**: 관심 종목을 국내 주식 / 국내 ETF / 미국 주식 / 미국 ETF 4가지 유형으로 즉시 필터링
...
- **주요 시장 지표 바**: 코스피, 코스닥, S&P500, 나스닥, 다우지수, SOX, VIX 실시간 표시. WTI+환율, 금+은은 각각 콤보 카드(이름·등락률 / 현재가 / 변동가 3줄)로 표시. 5개 지수 카드 클릭 시 차트 상세 페이지 이동. 그룹별 **전망 코멘트 스트립**: 지표 그룹 하단에 한 줄 마퀴(scrolling) 형태로 전망 문구 표시, 호버 시 전체 코멘트 툴팁 노출
...
- **관심 종목 탭**: 사용자가 추가한 종목/ETF 목록 (기본 활성 탭, 그룹별 분류, KRW/USD 토글, 미국 ETF 한글 설명 표시)
  - **종목 구분 필터**: 전체 / 국내 주식 / 국내 ETF / 미국 주식 / 미국 ETF 버튼으로 즉시 필터링. 소유자·그룹 필터와 AND 조건으로 결합. `getItemType()` 헬퍼가 `WatchlistItemDto.type` 및 종목명 정규식(`KODEX|TIGER|…`)으로 유형 자동 판별
  - **급등락 알림 패널 (±5%)**: 관심 종목 중 등락률 ±5% 이상 종목을 인라인 칩(chip) 형태로 탭 옆에 표시. 종목 클릭 시 차트 이동
...
- **시장지수 전용 분석 패널**: 시장 지수(코스피/코스닥/S&P500/나스닥/다우/SOX/VIX/WTI/환율/금/은) 차트에서는 매매 분석 대신 "시장 현황 분석" 패널 표시
  - 지수 소개 및 역할 설명
  - MA 20/50/200 대비 현황, 1개월·3개월 변화율, MA 배열 상태 테이블
  - 지수별 맞춤 현황 해석: VIX 구간별 시장 심리, 환율 구간별 영향 분석, WTI 수준, 금·SOX 추세 등
  - 관련 뉴스 인라인 표시 (키워드 자동 매핑)
- **이동평균선 분석 패널**: 크로스 시그널, 정배열/역배열 분석 (일반 종목 전용)
...
| `marketIndicators` | 5분 | 10 | 시장 지표 (코스피/코스닥/S&P500/나스닥/다우/SOX/VIX/WTI/환율/금/은) |
