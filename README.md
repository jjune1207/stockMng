...
- **주요뉴스**: 6개 RSS 소스(구글뉴스/다음/한국경제/연합뉴스/매일경제/이데일리)에서 설정 키워드 필터링, 36시간 이내 뉴스, 중복 제거 후 최대 10건 수집 (30분 자동 갱신)
...
- **주요뉴스 섹션**: 6개 RSS 소스에서 수집한 뉴스 가로 스크롤 카드 표시 (30분 갱신). 키워드 설정으로 필터링
- **주요 주식 탭**: 거래량 상위 주식 10개 (현재가, 등락률, 거래량)
- **주요 ETF 탭**: 거래량 상위 ETF 10개
- **미국 주식 탭**: S&P500 상위 10개 (USD/KRW 토글)
- **미국 ETF 탭**: 주요 ETF 15개 (USD/KRW 토글, 한글 설명 표시)
- **관심 종목 탭**: 사용자가 추가한 종목/ETF 목록 (그룹별 분류, USD/KRW 토글, 미국 ETF 한글 설명 표시)
...
| GET | `/api/stock/news?limit=10&keywords=미국,나스닥` | 주요 뉴스 (6개 RSS 소스, 30분 캐시, 기본값·최대 10건) |
...
| `usNews` | 30분 | 10 | 주요 뉴스 (6개 RSS 소스, 키워드 필터 + 중복 제거) |
...
| Google News RSS | 뉴스 키워드 필터링 한국어 뉴스 |
