...
- **해외 종목 지원**: Yahoo Finance 연동으로 미국 주식 시세·차트 조회, 원화/달러 가격 토글. 대표지수(^KS11/^KQ11 등) KRW 통화 자동 감지
- **미국 주요 뉴스**: 6개 RSS 소스(구글뉴스/다음/한국경제/연합뉴스/매일경제/이데일리)에서 미국 증시 키워드 필터링, 36시간 이내 뉴스, 중복 제거 후 최대 10건 수집 (30분 자동 갱신)
...
- **자동 갱신**: 주식/시장지표 5분, 뉴스 30분 별도 타이머 + 카운트다운 표시 + 수동 새로고침 버튼
...
| GET | `/api/stock/us-news?limit=10` | 미국 주요 뉴스 (6개 RSS 소스, 30분 캐시, 기본값·최대 10건) |
...
| `usNews` | 30분 | 10 | 미국 주요 뉴스 (6개 RSS 소스, 키워드 필터 + 중복 제거) |
...
| Yahoo Finance (`query1.finance.yahoo.com`) | 해외 종목(미국 주식 등) 시세·차트, 환율(USD/KRW), WTI, 해외 대표 지수. 통화 자동 감지(KRW/USD) |
| Google News RSS | 미국 증시 관련 한국어 뉴스 |
| 다음뉴스 / 한국경제 / 연합뉴스 / 매일경제 / 이데일리 RSS | 한국 경제 뉴스 (미국 증시 키워드 필터링) |
