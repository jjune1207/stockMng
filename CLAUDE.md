...
DB 없이 네이버 증권 API를 활용하여 거래량 상위 종목 조회, 관심 종목 관리, 기술적 분석(MA, 볼린저밴드) 및 종합 매매 분석(5개 지표)을 제공한다. 분봉(1/3/10분봉) 및 일봉 타임프레임 지원.
...
- **NaverStockServiceImpl**: 네이버 증권 API(시세, 검색, 거래량 랭킹, 분봉/일봉)를 WebClient로 호출. Caffeine 캐시 적용
...
- 외부 API: 네이버 증권 (시세, 차트, 검색, 랭킹) — URL은 `application.yml`의 `naver.*` 프로퍼티
- 캐시: Caffeine 인메모리 (현재가 30초, 캔들 10분, 분봉 1분, 검색 60분, 랭킹 10분)
...
모든 REST API는 `/api/stock` 하위. 캔들 조회: `/{symbol}/candle?timeframe=1|3|10|day` (분봉/일봉). 관심 종목 삭제는 복합키(`symbol|group`) 또는 symbol 단독 사용.
